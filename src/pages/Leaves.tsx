...
export default function Leaves() {
  ...
  const card = (l: LeaveRequest) => {
    ...
    return (
      <motion.div key={l.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-zinc-200 p-4 sm:p-5 shadow-sm">
        <div className="flex items-start gap-3">
          <UserAvatar p={p} size="h-10 w-10 text-xs" />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="font-bold text-sm">{p?.full_name || `#${l.employee_id}`}</p>
              <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase ${ST[l.status]}`}>{l.status}</span>
            </div>
            <p className="text-xs font-bold text-zinc-500 mt-1 flex items-center gap-1.5"><CalendarRange size={13} /> {l.from_date} → {l.to_date}</p>
        ...
        {isHead && l.status === 'pending' && (
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={() => decide(l.id, 'denied')} disabled={busy} className="flex-1 rounded-xl border border-red-300 text-red-600 text-sm font-bold py-2.5 hover:bg-red-50 disabled:opacity-60 flex items-center justify-center gap-1.5"><X size={16} /> Deny</button>
          </div>
        )}
      </motion.div>
    );
  ...
  return (
    <div className="space-y-4 sm:space-y-5 max-w-4xl">
      ...
      {isHead && (
        <>
          <h2 className="font-bold text-sm uppercase tracking-wider text-zinc-500">Pending ({pend.length})</h2>
          {pend.length === 0 ? <p className="text-sm text-zinc-500 bg-white border border-zinc-200 rounded-2xl p-5">No pending requests. 🎉</p> : <div className="space-y-3">{pend.map(card)}</div>}
          <h2 className="font-bold text-sm uppercase tracking-wider text-zinc-500 pt-2">Decided ({done.length})</h2>
          {done.length === 0 ? <p className="text-sm text-zinc-500">Nothing decided yet.</p> : <div className="space-y-3">{done.map(card)}</div>}
        </>
      )}
      {!isHead && (
        <div className="space-y-3">
          {list.length === 0 && <p className="text-sm text-zinc-500 bg-white border border-zinc-200 rounded-2xl p-6 text-center">No leave requests yet. Click “Request Leave” to apply.</p>}
          {list.map(card)}
        </div>
      )}
          ...
          <form onSubmit={submit} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              <div>
                ...
                <input type="date" required value={form.from_date} onChange={(e) => setForm({ ...form, from_date: e.target.value })} className="mt-1 w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
              </div>
              <div>
                <label className="text-xs font-bold text-zinc-500">TO</label>
                <input type="date" required value={form.to_date} min={form.from_date} onChange={(e) => setForm({ ...form, to_date: e.target.value })} className="mt-1 w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
              </div>
            </div>
            <textarea required value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} placeholder="Reason for leave…" rows={3} className="mt-3 w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
            <div className="flex gap-2 mt-4">
              ...
              <button disabled={busy} className="flex-1 rounded-xl bg-zinc-900 text-white font-bold py-3 text-sm hover:bg-zinc-700 disabled:opacity-60">{busy ? 'Sending…' : 'Submit Request'}</button>
            </div>
          </form>
        </div>
      )}
